...
std::string encrypt(const std::string &text) {
  ...
  for (const char &c : text) {
    encrypted += ciphers::morse::char_to_morse(c) + " ";
  }
...
std::string decrypt(const std::string &text) {
  ...
  std::size_t start = 0, end = 0, delim_len = 1;
  ...
  for (const std::string &s : splits) {
    decrypted += ciphers::morse::morse_to_char(s);
  }
 ...
 * @returns void
 */
// https://github.com/TheAlgorithms/C-Plus-Plus/blob/f9a1acd4c3ffebbb6f4b183869de1cb7fb508c17/ciphers/morse_code.cpp#L241
static void test() {
  // Test 1
